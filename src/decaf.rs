    ...
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        ...
        match self {
            ...
            BinArith(expr) => {
                write!(
                    f,
                    "binarith_{:?}_of_{}_and_{}",
                    expr.op, &*expr.lhs, &*expr.rhs
                )?;
            }
            UnArith(expr) => {
...
impl PartialEq for TypeBase {
    fn eq(&self, other: &Self) -> bool {
        ...
        match (self, other) {
            ...
            (UnknownTy(cls_a), UnknownTy(cls_b)) => cls_a.clone() == cls_b.clone(),
            _ => false,
        }
    }
}
    ...
    pub fn is_main_method(&self) -> bool {
        if self.name == "main" {
            if self.args.borrow().len() == 1 {
                ...
                match ty {
                    (_, ty) => match (&ty.base, ty.array_lvl) {
                        (ClassTy(cls), lvl) => {
...
pub struct UnamedVariable {
    // e.g. field, array access
}
...
impl ControlFlow for Stmt {
    fn returnable(&self) -> Option<Type> {
        ...
        match self {
            ...
            Declare(_) => None,
            If(_) => None,
            IfElse(stmt) => match (&*stmt.thenstmt).returnable() {
                None => None,
    ...
    fn branchable(&self) -> bool {
        ...
        match self {
            ...
            Break(_) => true,
            Super(_) => false,
            Block(blkstmt) => blkstmt
                .stmts
                .borrow()
                .iter()
                .rev()
                .any(|stmt| stmt.branchable()),
        }
    }
}
    ...
    pub fn variable_add(&self, name: String, ty: Type) {
        ...
        match self {
            ...
            MethodScope(method) => {
                ...
                println!(
                    "adding variable in method \"{}\" scope {}",
                    method.name, name
                );
...
impl Class {
    ...
    pub fn check_pub_method_defined_as_nonpub(
        &self,
        supr_pub_method: &Rc<Method>,
    ) -> Result<(), &'static str> {
    ...
    pub fn check_prot_method_defined_as_nonprot(
        &self,
        supr_prot_method: &Rc<Method>,
    ) -> Result<(), &'static str> {
    ...
    pub fn set_super(self: Rc<Class>, supr: Rc<Class>) {
        ...
        *self.sup_priv_fields.borrow_mut() = supr.priv_fields.borrow().clone();
        ...
        for supr_pub_method in supr.pub_methods.borrow().iter() {
            self.check_pub_method_defined_as_nonpub(supr_pub_method)
                .unwrap();
        }
        let pub_methods = prepend(
            &self.pub_methods.borrow()[..],
            &supr.pub_methods.borrow()[..],
        );
        *self.pub_methods.borrow_mut() = pub_methods;

        // Inherit protected methods
        for supr_prot_method in supr.prot_methods.borrow().iter() {
            self.check_prot_method_defined_as_nonprot(supr_prot_method)
                .unwrap();
        }
        let prot_methods = prepend(
            &self.prot_methods.borrow()[..],
            &supr.prot_methods.borrow()[..],
        );
    ...
    pub fn get_field_index(&self, field: &Rc<Field>) -> Option<u32> {
        ...
        match self.sup_pub_fields.borrow().iter().position(|f| {
            ...
            f.name == field.name
                && f.ty.borrow().is_compatible_with(&field.ty.borrow(), true)
                && f_cls_name.clone() == fld_cls_name.clone()
        }) {
            Some(ix) => {
    ...
    pub fn get_method_by_signature(
        ...
        let go = |methods: &Vec<Rc<Method>>| {
            'outer: for method in methods.iter().rev() {
                // Reverse order to get local method
    ...
    pub fn get_compatible_super_ctor(&self, args: &Vec<Expr>) -> Option<Rc<Ctor>> {
        ...
        match &*self.sup.borrow() {
            None => None,
            Some(sup_cls) => match go(&sup_cls.pub_ctors.borrow()) {
                Some(m) => Some(m),
    ...
    pub fn get_compatible_level0_static_method(
        &self,
        args: &Vec<Expr>,
        name: &str,
    ) -> Option<Rc<Method>> {
    ...
    pub fn get_compatible_level1_static_method(
        &self,
        args: &Vec<Expr>,
        name: &str,
    ) -> Option<Rc<Method>> {
    ...
    pub fn get_compatible_level0_super_method(
        &self,
        args: &Vec<Expr>,
        name: &str,
    ) -> Option<Rc<Method>> {
        ...
        match &*self.sup.borrow() {
            None => None,
            Some(sup_cls) => match go(&sup_cls.pub_methods.borrow()) {
                Some(m) => Some(m),
    ...
    pub fn get_compatible_level0_super_static_method(
        &self,
        args: &Vec<Expr>,
        name: &str,
    ) -> Option<Rc<Method>> {
        ...
        match &*self.sup.borrow() {
            None => None,
            Some(sup_cls) => match go(&sup_cls.pub_static_methods.borrow()) {
                Some(m) => Some(m),

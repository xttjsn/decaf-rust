...
impl Visitable for lnp::past::NNAExpr {
    fn accept<V: Visitor>(&self, visitor: &mut V) -> V::Result {
        visitor.visit_nnaexpr(self)
    }
}

impl Visitable for lnp::past::ActualArgs {
    fn accept<V: Visitor>(&self, visitor: &mut V) -> V::Result {
        visitor.visit_actural_args(self)
    }
}
...
trait ASTBuilder {
    ...
    fn type_add(
        &mut self,
        base_ty: Self::TypeBase,
        array_lvl: u32,
        hook: Option<Self::ClassHook>,
    ) -> Option<Self::Type>;
    ...
    fn normalize(&self) -> Self::Result {
        fn go(vid: &lnp::past::VarDeclaratorId, lvl: u32) -> (u32, String) {
            match &vid.id {
                ...
                lnp::past::VarDeclaratorIdInner::Array(vid_next) => go(&vid_next, lvl + 1),
            }
        }
...
fn ty2ty(ty: &lnp::past::Type, lvl: u32) -> (u32, decaf::TypeBase) {
    ...
    match ty {
        lnp::past::Type::Primitive(prim) => match prim {
        ...
        lnp::past::Type::Custom(cls_name) => {
            (lvl, decaf::TypeBase::UnknownTy(cls_name.to_string()))
        }
...
impl ASTBuilder for DecafTreeBuilder {
    ...
    fn type_add(
        ...
        match self.state {
            First => match base_ty {
                UnknownTy(cls_name) => match self.class_lookup(cls_name.clone()) {
                    None => {
    ...
    fn invoke_class_resolution_hook(&mut self, cls_name: String, cls: Rc<decaf::Class>) {
        fn go(
            class_hooks: &mut BTreeMap<String, Vec<DecafClassHook>>,
            cls_name: String,
            cls: Rc<decaf::Class>,
        ) {
            let mut tmp = vec![];
        ...
        *iocls.sup.borrow_mut() = Some(objcls.clone());
        {
            ...
            *put_string.args.borrow_mut() = vec![(
                "str".to_string(),
                Rc::new(Type {
            ...
            *peek.return_ty.borrow_mut() = Rc::new(Type {
            ...
            (*iocls.pub_static_methods.borrow_mut()).extend(vec![
                Rc::new(put_char),
                Rc::new(put_string),
                Rc::new(put_int),
                Rc::new(get_char),
...
impl Visitor for DecafTreeBuilder {
                ...
                if let Some(_) = self.class_lookup(cls_node.name.clone()) {
                    ...
                    match &cls_node.sup {
                        None => {
                            ...
                            new_class
                                .clone()
                                .set_super(self.class_lookup("Object".to_string()).unwrap());
                        }
                        Some(supnode) => {
                            match self.class_lookup(supnode.name.clone()) {
                                None => {
                                    ...
                                    self.add_class_resolution_hook(
                                        supnode.name.clone(),
                                        Box::new(move |cls| {
                    ...
                    self.scopes
                        .push(decaf::Scope::ClassScope(new_class.clone()));
                    ...
                    if !has_default_ctor {
                        ...
                        *default_ctor.body.borrow_mut() = Some(Rc::new(decaf::BlockStmt {
                    ...
                    if let Some(_) = *new_class.sup.borrow() {
                        self.invoke_class_resolution_hook(
        ...
        match &self.state {
            First => {
                ...
                for norm_field in field_node.normalize() {
                    ...
                    // Create field node
                    let field =
                        Rc::new(crate::decaf::Field::new(field_name, Some(curr_cls.clone())));
                    let tmp = field.clone();

                    // First check if we have change to reuse type
                    if let Some(field_type) = self.type_add(
                        ty,
                        array_lvl,
                        Some(Box::new(move |cls_ty| {
                ...
                for norm_field in field_node.normalize() {
                    ...
                    if let Some(init_expr) = init_expr {
                        match init_expr.accept(self) {
                            ...
                            _ => {
                                return Err(EExprNodeNotFound(format!(
                                    "init_expr of {}",
                                    field.name
                                )))
                            }
    ...
    fn visit_method(&mut self, mthd: &lnp::past::Method) -> Self::Result {
        match self.state {
            First => {
                ...
                // Get return type
                if let Some(return_ty) = self.type_add(
                    return_ty,
                    array_lvl,
                    Some(Box::new(move |cls_ty| {
                ...
                for (arg_name, arg_ty, arg_array_lvl) in args.into_iter() {
                    ...
                    if let Some(arg_ty) = self.type_add(
                        arg_ty.clone(),
                        arg_array_lvl,
                        Some(Box::new(move |cls_ty| {
                            tmp.set_arg_type(
                ...
                if curr_cls
                    ...
                    .borrow()
                    .iter()
                    .any(|m| m.has_same_signature(method_node.as_ref()))
                {
                ...
                match (mthd.modies.len(), mthd.modies.last()) {
                    ...
                    (2, _) => {
                        ...
                        match mthd.modies[0] {
                            ...
                            ModStatic(_) => match mthd.modies[1] {
                                ModPublic(_) => {
                                    *method_node.vis.borrow_mut() = Pub;
                                    *method_node.stat.borrow_mut() = true;
                                    curr_cls
            ...
            Second => {
                ...
                let is_static =
                    mthd.modies
                        .iter()
                        .any(|x| if let ModStatic(_) = x { true } else { false });

                // Resolve args
                args = args
                    .into_iter()
                    .map(|(name, ty_base, array_lvl)| {
                        (
                            name,
                            match ty_base {
                                UnknownTy(cls_name) => {
                                    ClassTy(self.class_lookup(cls_name).unwrap())
                                }
                                _ => ty_base,
                            },
                            array_lvl,
                ...
                // Get current method
                let method = match cls
                    .get_method_by_signature(&return_ty, array_lvl, &args, is_static, &name)
                {
                    Some(m) => m,
                    None => panic!(
                        "fail to get method by signature: {:?}_{}_{:?}_{}_{}",
                        return_ty, array_lvl, args, is_static, name
                    ),
                };
                ...
                let ret = match mthd.block.accept(self) {
                    ...
                    Ok(_) => Err(EBlockStmtNotFound(format!("{}", method))),
                    Err(e) => Err(e),
                };
    ...
    fn visit_expr(&mut self, expr: &lnp::past::Expression) -> Self::Result {
        match &expr.expr {
                ...
                match (right_expr.accept(self), left_expr.accept(self)) {
                    (Ok(rhs), Ok(lhs)) => {
                        ...
                        match lhs.get_type().is_compatible_with(&rhs.get_type(), false) {
                            ...
                            false => Err(EExprNotCompatibleType(format!(
                                "lhs={}, rhs={}",
                                lhs.get_type(),
                                rhs.get_type()
                            ))),
            ...
            UnaryExpr(unop, right_expr) => match right_expr.accept(self) {
                Ok(rhs) => {
                    let rhs = match rhs {
                        ExprNode(rhs) => rhs,
                    ...
                    match unop.op {
                        ...
                        NotUOp => {
                            if rhs.get_type().supports_logical_ops() {
                                Ok(ExprNode(UnNot(UnNotExpr { rhs: Box::new(rhs) })))
                            } else {
                                Err(EExprNotSupportLogicalOp)
                            }
                        }
        ...
        match self.state {
            First => {
                ...
                if curr_cls
                    ...
                    .borrow()
                    .iter()
                    .any(|m| m.has_same_signature(ctor_node.as_ref()))
                    || curr_cls
                        .prot_ctors
                        .borrow()
                        .iter()
            ...
            Second => {
                ...
                let ctor_node = match cls.get_ctor_by_signature(&args) {
                    Some(ctor) => ctor,
                    None => panic!(
                        "cannot find ctor for args: {}",
                        args.iter()
                            .map(|(name, ty, lvl)| format!(
                                "{}_{}",
                                name,
                                Type {
                                    base: ty.clone(),
                ...
                *ctor_node.body.borrow_mut() = Some(Rc::new(decaf::BlockStmt {
                ...
                let ret = match ctor.block.accept(self) {
                    ...
                    Ok(_) => Err(EBlockStmtNotFound(format!("{}", ctor_node))),
                    Err(e) => Err(e),
                };
                ...
                match &stmt.stmt {
                    ...
                    DeclareStmt(ty, var_decls) => {
                        ...
                        for var_decl in var_decls.iter() {
                            ...
                            match init_expr {
                                Some(init_expr) => match init_expr.accept(self) {
                                    Ok(init_expr_v) => match init_expr_v {
                                        ExprNode(init_expr_v) => {
                                            ...
                                            if init_expr_ty.is_compatible_with(&real_ty, false) {
                                                ...
                                                return Err(EExprNotCompatibleType(format!(
                                                    "init_expr_ty: {}, real_ty:{}",
                                                    init_expr_ty, real_ty
                                                )));
                                            }
                                        }
                    ...
                    IfStmt(condexpr, thenstmt) => match condexpr.accept(self) {
                        Ok(condexpr) => match condexpr {
                            ExprNode(condexpr) => {
                                if condexpr.get_type().supports_logical_ops() {
                                    match thenstmt.accept(self) {
                                        Ok(thenstmt) => match thenstmt {
                                            StmtNode(thenstmt) => Ok(StmtNode(If(decaf::IfStmt {
                                                cond: condexpr,
                                                thenstmt: Box::new(Block(Rc::new(
                                                    decaf::BlockStmt {
                                                        vartbl: RefCell::new(vec![]),
                    ...
                    IfElseStmt(condexpr, thenstmt, elsestmt) => match condexpr.accept(self) {
                        Ok(condexpr) => match condexpr {
                            ExprNode(condexpr) => {
                                if condexpr.get_type().supports_logical_ops() {
                                    match (thenstmt.accept(self), elsestmt.accept(self)) {
                                        (Ok(thenstmt), Ok(elsestmt)) => {
                    ...
                    ExprStmt(expr) => match expr.accept(self) {
                        Ok(expr) => match expr {
                            ExprNode(expr) => Ok(StmtNode(Expr(decaf::ExprStmt { expr }))),
                            _ => Err(EExprNodeNotFound(format!("expr stmt"))),
                        },

                        Err(e) => Err(e),
                    },
                    WhileStmt(condexpr, stmt) => match condexpr.accept(self) {
                        Ok(condexpr) => match condexpr {
                            ExprNode(condexpr) => {
                                ...
                                match self.get_top_most_method_scope() {
                                    Some(MethodScope(method)) => {
                                        if let VoidTy = &(*method.return_ty.borrow()).base {
                                            Ok(StmtNode(Return(decaf::ReturnStmt { expr: None })))
                                        } else {
                            ...
                            Some(retexpr) => match retexpr.accept(self) {
                                Ok(retexpr) => match retexpr {
                                    ExprNode(retexpr) => {
                                        ...
                                        match self.get_top_most_method_scope() {
                                            Some(MethodScope(method)) => {
                                                if ret_ty.is_compatible_with(
                                                    &method.return_ty.borrow(),
                                                    false,
                                                ) {
                                                    ...
                                                    Err(EUnmatchedReturnType(format!(
                                                        "{}",
                                                        retexpr
                                                    )))
                                                }
                    ...
                    BreakStmt => {
                        ...
                        // Check if we are in a loop
                        match self.get_top_most_while_scope() {
                            Some(WhileScope(whilestmt)) => Ok(StmtNode(Break(decaf::BreakStmt {
                                lup: whilestmt.clone(),
                            }))),
                                        ...
                                        match self.get_top_most_class_scope() {
                                            Some(ClassScope(cls)) => {
                                                match cls.get_compatible_super_ctor(&args_expr) {
                                                    Some(sup_ctor) => {
                                                        Ok(StmtNode(Super(decaf::SuperStmt {
                                                            sup: cls
                                                                .sup
                                                                .borrow()
                                                                .as_ref()
                ...
                match &prim.prim {
                    NewArray(naexpr) => {
                        match &naexpr.expr {
                                    ...
                                    Ok(dims_expr) => {
                                        match dims_expr {
                                            VecExpr(dims_expr) => {
                                                if dims_expr
                                                    .iter()
                                                    .all(|dim| dim.get_type().supports_arith_ops())
                                                {
                                                    // lookup class for id
                            ...
                            NewPrimitive(primtype, dims) => match dims.accept(self) {
                                Ok(dims_expr) => match dims_expr {
                                    VecExpr(dims_expr) => {
                                        if dims_expr
                                            .iter()
                                            .all(|dim| dim.get_type().supports_arith_ops())
                    ...
                    Identifier(id) => {
                        match self.variable_lookup(id.clone()) {
                            // look for variable
                            Some(var) => Ok(ExprNode(Variable(VariableExpr { var }))),
                            None => {
                                // Look for fields
                                match self.get_top_most_class_scope() {
                                    Some(ClassScope(cls)) => match cls.get_field_by_name(&id) {
                                        Some(field) => Ok(ExprNode(FieldAccess(FieldAccessExpr {
                                            var: Box::new(This(ThisExpr { cls: cls.clone() })),
                                            cls,
                                            fld: field,
                                        }))),
                                        None => match self.class_lookup(id.clone()) {
                                            Some(cls) => Ok(ExprNode(ClassId(ClassIdExpr { cls }))),
            ...
            Second => {
                match &nnaexpr {
                    JustLit(litr) => match &litr.litr {
                        ...
                        Str(ref s) => match self.class_lookup("String".to_string()) {
                            Some(cls) => {
                                        ...
                                        match self.get_top_most_method_scope() {
                                            Some(MethodScope(method)) => {
                                                match *method.stat.borrow() {
                                                    ...
                                                    true => {
                                                        match cls
                                                            .get_compatible_level0_static_method(
                                                                &arg_exprs, id,
                                                            ) {
                                                            Some(method) => Ok(ExprNode(
                                ...
                                match nprim.accept(self) {
                                    Ok(ExprNode(prim_expr)) => {
                                        match prim_expr {
                                            ClassId(cls_id_expr) => {
                                                match if self
                                                    .has_class_in_scope_stack(&cls_id_expr.cls)
                                                {
                                                    cls_id_expr
                                                        .cls
                                                        .get_compatible_level0_static_method(
                                                    ...
                                                    cls_id_expr
                                                        .cls
                                                        .get_compatible_level1_static_method(
                                                            &arg_exprs, id,
                                                        )
                                                } {
                                                    Some(method) => {
                                                        if method.name == *id {
                                                            Ok(ExprNode(MethodCall(
                                                                MethodCallExpr {
                                                                    var: Box::new(ClassId(
                                                                        cls_id_expr,
                                                                    )),
                                            ...
                                            _ => {
                                                ...
                                                if let ClassTy(cls) = prim_ty.base {
                                                    if prim_ty.array_lvl == 0 {
                                                        ...
                                                        match if self.has_class_in_scope_stack(&cls)
                                                        {
                                                            cls.get_compatible_level0_method(
                                                                &arg_exprs, id,
                                                            )
                                                        } else {
                                                            cls.get_compatible_level1_method(
                                                                &arg_exprs, id,
                                                            )
                                                        } {
                                                            Some(method) => {
                                ...
                                match self.get_top_most_class_scope() {
                                    Some(ClassScope(cls)) => {
                                        match cls.get_compatible_level0_super_method(&arg_exprs, id)
                                        {
                                            ...
                                            None => match cls
                                                .get_compatible_level0_super_static_method(
                                                    &arg_exprs, id,
                                                ) {
                                                Some(method) => {
                                                    Ok(ExprNode(MethodCall(MethodCallExpr {
                    ...
                    EvalArray(arrexpr) => {
                        match &arrexpr.expr {
                            SimpleArraryExpr(id, dim) => {
                                ...
                                match self.variable_lookup(id.clone()) {
                                    Some(inst) => match dim.expr.accept(self) {
                                        ...
                                        match self.get_top_most_class_scope() {
                                            Some(ClassScope(cls)) => {
                                                match cls.get_field_by_name(&id) {
                                                    Some(field) => match dim.expr.accept(self)? {
                                                        ExprNode(idx_expr) => Ok(ExprNode(
                                                            ArrayAccess(ArrayAccessExpr {
                                                                var: Box::new(FieldAccess(
                                                                    FieldAccessExpr {
                                                                        var: Box::new(This(
                                                                            ThisExpr {
                                                                                cls: cls.clone(),
                                                        ...
                                                        _ => Err(EExprNodeNotFound(format!(
                                                            "Field lookup"
                                                        ))),
                                                    },
                                                    None => Err(EUnknownIdentifier(id.clone())),
                            ...
                            ComplexArrayExpr(nna, dim) => match nna.expr.accept(self) {
                                Ok(prim_expr) => match prim_expr {
                                        ...
                                        if ty.array_lvl > 0 {
                                            match dim.expr.accept(self) {
                                                Ok(dim_expr) => match dim_expr {
                                                    ...
                                                    _ => {
                                                        Err(EExprNodeNotFound(format!("dim expr")))
                                                    }
                                                },
                                                Err(e) => Err(e),
                                            }
                                    ...
                                    _ => {
                                        Err(EExprNodeNotFound(format!("ComplexArrayExpr PrimExpr")))
                                    }
                                },
                                Err(e) => Err(e),
                            },
                                                        ...
                                                        _ => {
                                                            if ty.array_lvl > 0 {
                                                                if id == "length" {
                                                                    // Only field "length"
                                                                    let fld =
                                                                        Rc::new(decaf::Field::new(
                                                                            "length".into(),
                                                                            None,
                                                                        ));
                                                                    fld.set_base_type(Rc::new(
                                                                        decaf::Type {
                                                                            base: IntTy,
                                                                            array_lvl: 0,
                                                                        },
    ...
    fn visit_this(&mut self) -> Self::Result {
        match self.get_top_most_class_scope() {
            Some(ClassScope(cls)) => match self.get_top_most_method_scope() {
...
pub fn parse_decaf<'a>(src: &str) -> Result<Program, String> {
    ...
    match builder.visit_program(&syntactic_program) {
        Ok(Normal) => Ok(Program {
            classes: builder
                .class_map
                .iter()
                .map(|(_, cls)| cls.clone())
                .collect::<Vec<Rc<decaf::Class>>>(),
        }),

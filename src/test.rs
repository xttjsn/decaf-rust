	...
	private static int sfoobar() {}
        ...
        match builder.class_map.get(&"Main".to_string()) {
            ...
            Some(c) => {
                ...
                assert_eq!(
                    c.priv_static_methods.borrow().last().unwrap().name,
                    "sfoobar"
                );
            }
        }
...
"#;
        ...
        match builder.class_map.get(&"Woo".to_string()) {
            ...
            Some(woo) => {
                ...
                println!(
                    "fileds: {}, {}",
                    woo.pub_fields.borrow()[0].name,
                    woo.pub_fields.borrow()[1].name
                );
                ...
                let stmt_x_eq_0 = &block.stmts.borrow()[1];
                {
                    ...
                    match stmt_x_eq_0 {
                        Stmt::Expr(stmt) => {
                            let expr = &stmt.expr;
                            match expr {
                                Expr::Assign(expr) => match (&*expr.lhs, &*expr.rhs) {
                                ...
                                Expr::Assign(expr) => match (&*expr.lhs, &*expr.rhs) {
                                    (Expr::Variable(var_expr), Expr::BinArith(ba_expr)) => {
                                        ...
                                        match (&*ba_expr.lhs, &*ba_expr.rhs) {
                                            ...
                                            _ => panic!(
                                                "lhs, rhs expected to be Variable and Literal"
                                            ),
                                        }
                                    }
                                ...
                                Expr::Assign(expr) => match (&*expr.lhs, &*expr.rhs) {
                                    (Expr::Variable(var_expr), Expr::BinArith(ba_expr)) => {
                                        ...
                                        match (&*ba_expr.lhs, &*ba_expr.rhs) {
                                            ...
                                            _ => panic!(
                                                "lhs, rhs expected to be Variable and Variable"
                                            ),
                                        }
                                    }
                                    _ => panic!("lhs, rhs expected to be Variable and BinArith"),
                            ...
                            match expr {
                                Expr::BinCmp(expr) => {
                                    ...
                                    match (&*expr.lhs, &*expr.rhs) {
                                        ...
                                        _ => panic!(
                                            "lhs, rhs expected to be Variable and IntLiteral"
                                        ),
                                    }
                                }
                                _ => panic!("expr expected to be BinCmp expr"),
                            ...
                            match expr {
                                Expr::BinCmp(expr) => {
                                    ...
                                    match (&*expr.lhs, &*expr.rhs) {
                                        ...
                                        _ => {
                                            panic!("lhs, rhs expected to be Variable and Variable")
                                        }
                                    }
                                }
                                _ => panic!("expr expected to be BinCmp expr"),
                        ...
                        Stmt::Expr(stmt) => {
                            ...
                            match expr {
                                Expr::BinLogical(expr) => {
                                    ...
                                    match (&*expr.lhs, &*expr.rhs) {
                                        (
                                            Expr::BinCmp(expr_x_eqs_0),
                                            Expr::BinCmp(expr_x_neqs_10),
                                        ) => {
                                            ...
                                            match (
                                                ...
                                                (
                                                    ...
                                                    match lit_0 {
                                                        ...
                                                        _ => panic!(
                                                            "lit expected to be a LiteralExpr of 0"
                                                        ),
                                                    }

...
use llvm_sys::{
    core::*, prelude::*, target::*, target_machine::*, transforms::pass_manager_builder::*,
    LLVMIntPredicate::*,
};
...
use crate::decaf;
use crate::decaf::{
    BlockStmt, Class, ControlFlow, Expr::*, LiteralExpr::*, Scope::*, Stmt::*, TypeBase::*, VTable,
    Value, Variable, VariableTable,
};
use crate::shell;
...
impl DecafValue {
    pub fn get_type(&self) -> decaf::Type {
        match self {
            Addr(val) => val.ty.clone(),
            Val(ty, _) => ty.clone(),
        }
    }
    pub fn get_value(&self, generator: &mut CodeGenerator) -> LLVMValueRef {
        match self {
            Addr(val) => match &*val.addr.borrow() {
                Some(addr) => unsafe {
                    LLVMBuildLoad(
                        generator.builder,
                        addr.clone(),
                        generator.new_string_ptr(&format!(
                            "{}_ref_{}",
                            val.name,
                            val.next_refcount()
                        )),
                ...
                None => panic!("variable \"{}\" address not set", val.name),
            },
            Val(_, val) => val.clone(),
        }
    }
...
impl CodeGenerator {
    ...
    fn char_type(&self) -> LLVMTypeRef {
        unsafe { LLVMInt8TypeInContext(LLVMGetModuleContext(self.module)) }
    }

    fn int_type(&self) -> LLVMTypeRef {
        unsafe { LLVMInt64TypeInContext(LLVMGetModuleContext(self.module)) }
    }

    fn bool_type(&self) -> LLVMTypeRef {
        unsafe { LLVMInt1TypeInContext(LLVMGetModuleContext(self.module)) }
    }

    fn i32_type(&self) -> LLVMTypeRef {
        unsafe { LLVMInt32TypeInContext(LLVMGetModuleContext(self.module)) }
    }

    fn void_type(&self) -> LLVMTypeRef {
        unsafe { LLVMVoidTypeInContext(LLVMGetModuleContext(self.module)) }
    }

    fn i64_ptr_type(&self) -> LLVMTypeRef {
        unsafe { LLVMPointerType(self.int_type(), ADDRESS_SPACE_GENERIC) }
    }

    fn char_ptr_type(&self) -> LLVMTypeRef {
        unsafe { LLVMPointerType(self.char_type(), ADDRESS_SPACE_GENERIC) }
    }

    fn const_i64(&self, val: i64) -> LLVMValueRef {
        unsafe { LLVMConstInt(self.int_type(), val as u64, 0) }
    }

    fn const_bool(&self, val: bool) -> LLVMValueRef {
        unsafe { LLVMConstInt(self.bool_type(), val as u64, 0) }
    }

    fn const_char(&self, val: char) -> LLVMValueRef {
        unsafe { LLVMConstInt(self.char_type(), val as u64, 0) }
    }

    fn indices(&self, args: Vec<i64>) -> Vec<LLVMValueRef> {
        unsafe {
            args.iter()
                .map(|v| LLVMConstInt(self.i32_type(), (*v) as u64, 0))
    ...
    fn ptr_if_class_or_array(&self, ty: &decaf::Type) -> LLVMTypeRef {
        unsafe {
            match ty.base {
                ClassTy(_) => ptr_of!(self.get_llvm_type_from_decaf_type(ty)),
                _ => {
                    if ty.array_lvl > 0 {
    ...
    fn get_llvm_type_from_decaf_type(&self, ty: &decaf::Type) -> LLVMTypeRef {
        ...
        let (base_llvm_ty, is_class) = match &ty.base {
            ...
            ClassTy(cls) => (self.get_llvm_type_by_name(&cls.name).unwrap(), true),
        };
    ...
    fn get_top_most_block(&self) -> Option<Rc<BlockStmt>> {
        for scope in self.scopes.iter().rev() {
            match scope {
                BlockScope(blk) => return Some(blk.clone()),
                _ => continue,
            }
        }
        None
    ...
    fn get_top_most_class(&self) -> Option<Rc<Class>> {
        for scope in self.scopes.iter().rev() {
            match scope {
                ClassScope(cls) => return Some(cls.clone()),
                _ => continue,
            }
        }
        None
        ...
        if src_ty.is_compatible_with(dst_ty, false) {
            ...
            if is_ptr {
                if src_ty.array_lvl == dst_ty.array_lvl {
                    if src_ty.base != dst_ty.base {
                        unsafe {
                            ...
                            match (&src_ty.base, &dst_ty.base) {
                                ...
                                (ClassTy(_), IntTy) | (ClassTy(_), NULLTy) => LLVMBuildPtrToInt(
                                    self.builder,
                                    val,
                                    dst_llvm_ty,
                                    self.next_name(&format!("cast_{}_{}", src_ty, dst_ty)),
                    ...
                    panic!(
                        "casting will change array_lvl: src_type: {}, dst_type: {}",
                        src_ty, dst_ty
                    );
                }
            } else {
            ...
            panic!(
                "casting incompatible types. src_type: {}, dst_type: {}",
                src_ty, dst_ty
            );
        }
    }
    ...
    fn new_string_class(&mut self, s: &str) -> LLVMValueRef {
        ...
        unsafe {
            // Allocate memory
            let string_val = LLVMBuildMalloc(
                self.builder,
                self.get_llvm_type_by_name("String").unwrap(),
                self.next_name("malloc_string"),
            );

            // Copy string
            let chars = CString::new(s).unwrap();
            let chars = chars.as_bytes_with_nul();
            let str_addr = LLVMBuildGEP(
                self.builder,
                string_val,
                self.indices(vec![0, 0]).as_mut_slice().as_mut_ptr() as *mut _,
                2,
                self.next_name("gep_string_inner"),
            );
            let s_ptr = LLVMBuildGlobalStringPtr(
                self.builder,
                chars.as_ptr() as *const _,
                self.next_name("const_string_2"),
            );
    ...
    pub fn next_name(&mut self, name: &str) -> *const i8 {
        *self.names.entry(name.into()).or_insert(0).borrow_mut() += 1;
        self.new_string_ptr(&format!(
            "{}_{}",
            name,
            self.names.get(name.into()).unwrap()
        ))
    }
    ...
    fn gen_builtin(&mut self) {
        ...
        unsafe {
            ...
            // putChar
            {
                ...
                let put_char_inner_ty =
                    LLVMFunctionType(self.void_type(), params!(self.char_type()), 1, 0);
                let put_char_inner_val =
                    LLVMAddFunction(self.module, IO_PUT_CHAR_NAME, put_char_inner_ty);

                // Called by decaf code
                let put_char_ty =
                    LLVMFunctionType(self.void_type(), params!(self.char_type()), 1, 0);
                let put_char_val =
                    LLVMAddFunction(self.module, name!(b"IO_putChar\0"), put_char_ty);
                let put_char_bb = LLVMAppendBasicBlockInContext(
                    LLVMGetModuleContext(self.module),
                    put_char_val,
                    name!(b"IO_putChar_main\0"),
                );
                ...
                LLVMBuildCall(
                    self.builder,
                    put_char_inner_val,
                    params!(char_param_val),
                    1,
                    name!(b"\0"),
                );
                LLVMBuildRetVoid(self.builder);

                self.add_function("IO_pub_static_method_putChar_char_0_void_0", put_char_val)
                    .unwrap();
            }
            ...
            {
                // Interface of Runtime
                let put_string_inner_ty =
                    LLVMFunctionType(self.void_type(), params!(self.char_ptr_type()), 1, 0);
                let put_string_inner_val =
                    LLVMAddFunction(self.module, IO_PUT_STRING_NAME, put_string_inner_ty);

                // Wrapper method
                let put_string_ty = LLVMFunctionType(
                    self.void_type(),
                    params!(ptr_of!(self.get_llvm_type_by_name("String").unwrap())),
                    1,
                    0,
                );
                let put_string_val =
                    LLVMAddFunction(self.module, name!(b"IO_putString\0"), put_string_ty);
                let put_string_bb = LLVMAppendBasicBlockInContext(
                    LLVMGetModuleContext(self.module),
                    put_string_val,
                    name!(b"IO_putString_main\0"),
                );
                ...
                LLVMBuildRetVoid(self.builder);
                self.add_function(
                    "IO_pub_static_method_putString_String_0_void_0",
                    put_string_val,
                )
                .unwrap();
            }
            ...
            {
                // Interface of Runtime
                let put_int_inner_ty =
                    LLVMFunctionType(self.void_type(), params!(self.int_type()), 1, 0);
                let put_int_inner_val =
                    LLVMAddFunction(self.module, IO_PUT_INT_NAME, put_int_inner_ty);
                ...
                let put_int_bb = LLVMAppendBasicBlockInContext(
                    LLVMGetModuleContext(self.module),
                    put_int_val,
                    name!(b"IO_putInt_main\0"),
                );
                ...
                LLVMBuildCall(
                    self.builder,
                    put_int_inner_val,
                    params!(int_param_val),
                    1,
                    name!(b"\0"),
                );
                LLVMBuildRetVoid(self.builder);
                self.add_function("IO_pub_static_method_putInt_int_0_void_0", put_int_val)
                    .unwrap();
            }
            ...
            // getChar
            {
                ...
                let get_char_inner_val =
                    LLVMAddFunction(self.module, IO_GET_CHAR_NAME, get_char_inner_ty);

                // Called by decaf code
                let get_char_ty = LLVMFunctionType(self.char_type(), null!(), 0, 0);
                let get_char_val =
                    LLVMAddFunction(self.module, name!(b"IO_getChar\0"), get_char_ty);
                let get_char_bb = LLVMAppendBasicBlockInContext(
                    LLVMGetModuleContext(self.module),
                    get_char_val,
                    name!(b"IO_getChar_main\0"),
                );
                LLVMPositionBuilderAtEnd(self.builder, get_char_bb);
                let char_val = LLVMBuildCall(
                    self.builder,
                    get_char_inner_val,
                    null!(),
                    0,
                ...
                self.add_function("IO_pub_static_method_getChar__char_0", get_char_val)
                    .unwrap();
            }
            ...
            // getLine
            {
                ...
                let get_line_inner_val =
                    LLVMAddFunction(self.module, IO_GET_LINE_NAME, get_line_inner_ty);
                LLVMSetLinkage(get_line_inner_val, LLVMExternalLinkage);

                // Called by decaf code
                let get_line_ty = LLVMFunctionType(
                    ptr_of!(self.get_llvm_type_by_name("String").unwrap()),
                    null!(),
                    0,
                    0,
                );
                let get_line_val =
                    LLVMAddFunction(self.module, name!(b"IO_getLine\0"), get_line_ty);
                let get_line_bb = LLVMAppendBasicBlockInContext(
                    LLVMGetModuleContext(self.module),
                    get_line_val,
                    name!(b"IO_getLine_main\0"),
                );
                LLVMPositionBuilderAtEnd(self.builder, get_line_bb);
                let inner_line = LLVMBuildCall(
                    self.builder,
                    get_line_inner_val,
                    null!(),
                    0,
                ...
                let str_addr = LLVMBuildGEP(
                    ...
                    self.indices(vec![0, 0]).as_mut_slice().as_mut_ptr() as *mut _,
                    2,
                    name!(b"String_field_str\0"),
                );
                LLVMBuildStore(self.builder, inner_line, str_addr);
                LLVMBuildRet(self.builder, string_val);

                self.add_function("IO_pub_static_method_getLine__String_0", get_line_val)
                    .unwrap();
            }
            ...
            // getInt
            {
                ...
                let get_int_inner_val =
                    LLVMAddFunction(self.module, IO_GET_INT_NAME, get_int_inner_ty);
                ...
                let get_int_bb = LLVMAppendBasicBlockInContext(
                    LLVMGetModuleContext(self.module),
                    get_int_val,
                    name!(b"IO_getInt_main\0"),
                );
                LLVMPositionBuilderAtEnd(self.builder, get_int_bb);
                let inner_int = LLVMBuildCall(
                    self.builder,
                    get_int_inner_val,
                    null!(),
                    0,
                ...
                self.add_function("IO_pub_static_method_getInt__int_0", get_int_val)
                    .unwrap();
            }
            ...
            // peek
            {
                ...
                let peek_bb = LLVMAppendBasicBlockInContext(
                    LLVMGetModuleContext(self.module),
                    peek_val,
                    name!(b"IO_peek_main\0"),
                );
                LLVMPositionBuilderAtEnd(self.builder, peek_bb);
                let peek_char = LLVMBuildCall(
                    self.builder,
                    peek_inner_val,
                    null!(),
                    0,
                ...
                self.add_function("IO_pub_static_method_peek__char_0", peek_val)
                    .unwrap();
            }
        }
    }
    ...
    pub fn compile_to_module(&mut self, program: &Program) -> Result<LLVMModuleRef, String> {
        ...
        if self.mains.len() == 0 {
            ...
            unsafe {
                ...
                let main_val =
                    LLVMAddFunction(self.module, self.new_string_ptr("DECAF_MAIN"), main_ty);
                let main_bb = LLVMAppendBasicBlockInContext(
                    LLVMGetModuleContext(self.module),
                    main_val,
                    name!(b"decaf_main_bb\0"),
                );
                LLVMPositionBuilderAtEnd(self.builder, main_bb);
                let arg_val = LLVMGetFirstParam(main_val);
                LLVMBuildCall(
                    self.builder,
                    *self.mains.last().unwrap(),
                    params!(arg_val),
                    1,
                    name!(b"\0"),
                );
    ...
    fn gencode(self: Rc<Self>, generator: &mut CodeGenerator) -> GenCodeResult {
        ...
        match generator.state {
            GenState::First => {
                ...
                unsafe {
                    vtable_ty =
                        LLVMArrayType(generator.i64_ptr_type(), self.vtable.borrow().len() as u32);
                    vtable_global = LLVMAddGlobal(
                        generator.module,
                        vtable_ty,
                ...
                for field in all_fields.iter() {
                    match field.ty.borrow().base {
                        ...
                        _ => {
                            field_llvm_types
                                .push(generator.ptr_if_class_or_array(&*field.ty.borrow()));
                        }
                    }
                }
                ...
                let cls_llvm_type;
                unsafe {
                    cls_llvm_type = LLVMStructType(
                        field_llvm_types.as_mut_ptr() as *mut _,
            ...
            GenState::Second => {
                ...
                let mut vtable_vec: Vec<Option<LLVMValueRef>> =
                    vec![None; self.vtable.borrow().len()];
                ...
                unsafe {
                    ...
                    for ctor in ctors.iter() {
                        ...
                        ctor_ty = LLVMFunctionType(
                            ptr_of!(cls_llvm_type.clone()),
                            args.as_mut_ptr() as *mut _,
                            args.len() as u32,
                            0,
                        );
                        ctor_val = LLVMAddFunction(
                            generator.module,
                    ...
                    for method in self.vtable.borrow().iter() {
                        let method_val = {
                            if !generator.has_function(&method.llvm_name()) {
                                let method_ty;
                                let method_val;
                                let possibly_super_cls_llvm_type = generator
                                    .get_llvm_type_by_name(&method.cls.borrow().name)
                                    .unwrap();
                                ...
                                let return_ty =
                                    generator.ptr_if_class_or_array(&*method.return_ty.borrow());
                                method_ty = LLVMFunctionType(
                                    return_ty,
                                    args.as_mut_ptr() as *mut _,
                                    args.len() as u32,
                                    0,
                                );
                                method_val = LLVMAddFunction(
                                    generator.module,
                    ...
                    for priv_method in self.priv_methods.borrow().iter() {
                        ...
                        let return_ty =
                            generator.ptr_if_class_or_array(&*priv_method.return_ty.borrow());
                        method_ty = LLVMFunctionType(
                            return_ty,
                            args.as_mut_ptr() as *mut _,
                            args.len() as u32,
                    ...
                    for staticm in all_statics.iter() {
                        ...
                        static_ty = LLVMFunctionType(
                            generator.ptr_if_class_or_array(&*staticm.return_ty.borrow()),
                            args.as_mut_ptr() as *mut _,
                            staticm.args.borrow().len() as u32,
                            0,
                        );
                        static_val = LLVMAddFunction(
                            generator.module,
                        ...
                        match *staticm.stat.borrow() {
                            true => match staticm.return_ty.borrow().base {
                                VoidTy => {
                ...
                // Set vtable
                unsafe {
                    let vtable_value = LLVMConstArray(
                            ...
                            .map(|(ix, v)| {
                                LLVMConstBitCast(
                                    match v {
                                        Some(f) => f.clone(),
                                        None => panic!(
                                            "The {}-th function is not defined for {}",
                                            ix, self.name
                                        ),
                                    },
                                    generator.i64_ptr_type(),
                    ...
                    let vtable_global = generator
                        .get_global_by_name(&vtable_name!(self.name))
                        .unwrap();
            ...
            Some(ctor_llvm_val) => {
                ...
                unsafe {
                    let ctor_bb = LLVMAppendBasicBlockInContext(
                        ...
                        ctor_llvm_val,
                        generator.new_string_ptr(&format!("{}_mainbb", self.llvm_name())),
                    );
                ...
                unsafe {
                    ...
                    LLVMBuildStore(generator.builder, this_val, this_addr);
                    self.vartbl
                        .borrow_mut()
                        .set_addr_for_name("this", this_addr);
                    ...
                    for (ix, (arg_name, _)) in self.args.borrow().iter().enumerate() {
                        ...
                        LLVMBuildStore(generator.builder, arg_val, arg_addr);

                        self.vartbl
                            .borrow_mut()
                            .set_addr_for_name(arg_name, arg_addr);
                    }
                }
                ...
                if let None = block.returnable() {
                    ...
                    unsafe {
                        let last_inst =
                            LLVMGetLastInstruction(LLVMGetInsertBlock(generator.builder));
                        ...
                        if let None = block.returnable() {
                            ...
                            unsafe {
                                ...
                                if no_ret {
                                    ...
                                    let casted_val = generator.cast_value(
                                        &(&IntTy).into(),
                                        self.return_ty.borrow().as_ref(),
                                        val,
                                    );
...
impl CodeGen for decaf::Stmt {
    fn gencode(&self, generator: &mut CodeGenerator) -> GenCodeResult {
        ...
        match self {
            Declare(stmt) => {
                let lhs_addr = {
                    ...
                    unsafe {
                        LLVMBuildAlloca(
                            generator.builder,
                            generator.ptr_if_class_or_array(&stmt.ty),
                ...
                blk.vartbl
                    .borrow_mut()
                    .set_addr_for_name(&stmt.name, lhs_addr);

                match &stmt.init_expr {
                    Some(expr) => match expr.gencode(generator)? {
                        CodeValue::Value(val) => {
                            if stmt.ty.is_compatible_with(&val.get_type(), false) {
                                unsafe {
                                    let llvm_val = val.get_value(generator);
                                    let casted_val =
                                        generator.cast_value(&expr.get_type(), &stmt.ty, llvm_val);
                ...
                match generator.get_top_most_function_val() {
                    Some(func_val) => {
                        ...
                        match stmt.cond.gencode(generator)? {
                            CodeValue::Value(cond_val) => unsafe {
            ...
            IfElse(stmt) => match generator.get_top_most_function_val() {
                Some(func_val) => match stmt.cond.gencode(generator)? {
                    CodeValue::Value(cond_val) => unsafe {
                        let ifbb = LLVMAppendBasicBlockInContext(
                            LLVMGetModuleContext(generator.module),
                            func_val,
                            ...
                            *stmt.nextbb.borrow_mut() = Some(nextbb.clone());
                            ...
                            match stmt.condexpr.gencode(generator)? {
                                CodeValue::Value(cond_val) => {
                                    ...
                                    Block(stmt.bodyblock.borrow().clone().unwrap())
                                        .gencode(generator)?;
            ...
            Return(stmt) => match generator.get_top_most_function_return_ty() {
                Some(func_return_ty) => unsafe {
            ...
            Continue(stmt) => match stmt.lup.condbb.borrow().clone() {
                Some(condbb) => unsafe {
                    LLVMBuildBr(generator.builder, condbb);
                    Ok(OK)
                },
                None => Err(ELoopLacksCondBB(format!("{}", self))),
            },
            Break(stmt) => match stmt.lup.nextbb.borrow().clone() {
                Some(nextbb) => unsafe {
                    LLVMBuildBr(generator.builder, nextbb);
                    Ok(OK)
                },
...
impl CodeGen for decaf::Expr {
    fn gencode(&self, generator: &mut CodeGenerator) -> GenCodeResult {
        match &*self {
                ...
                match (&*expr.rhs).gencode(generator)? {
                    CodeValue::Value(val_rhs) => {
                        match (&*expr.lhs).gencode(generator)? {
                            ...
                            CodeValue::Value(_) => {
                                Err(ENonAddressableValueOnLHS(format!("{}", expr.lhs)))
                            }
            ...
            BinArith(expr) => {
                match (
                    (&*expr.lhs).gencode(generator)?,
                    (&*expr.rhs).gencode(generator)?,
                ) {
                    (CodeValue::Value(val_lhs), CodeValue::Value(val_rhs)) => {
                        ...
                        unsafe {
                            ...
                            let lhs_casted = generator.cast_value(
                                &val_lhs.get_type(),
                                &(&IntTy).into(),
                                lhs_llvm_val,
                            );
                            let rhs_casted = generator.cast_value(
                                &val_rhs.get_type(),
                                &(&IntTy).into(),
                                rhs_llvm_val,
                            );
                            match expr.op {
                                PlusOp => Ok(CodeValue::Value(Val(
                                    val_rhs.get_type(),
                                    LLVMBuildAdd(
                                        generator.builder,
                                        lhs_casted,
                                ...
                                TimesOp => Ok(CodeValue::Value(Val(
                                    val_rhs.get_type(),
                                    LLVMBuildMul(
                                        generator.builder,
                                        lhs_casted,
                                ...
                                DivideOp => Ok(CodeValue::Value(Val(
                                    val_rhs.get_type(),
                                    LLVMBuildSDiv(
                                        generator.builder,
                                        lhs_casted,
            ...
            UnArith(expr) => {
                ...
                match (&*expr.rhs).gencode(generator)? {
                    CodeValue::Value(val_rhs) => unsafe {
                        let rhs_llvm_val = val_rhs.get_value(generator);
                        let rhs_casted = generator.cast_value(
                            &val_rhs.get_type(),
                            &(&IntTy).into(),
                        ...
                        match expr.op {
                            PlusUOp => Ok(CodeValue::Value(Val(val_rhs.get_type(), rhs_casted))),
                            MinusUOp => Ok(CodeValue::Value(Val(
                                val_rhs.get_type(),
                                LLVMBuildNeg(
                                    generator.builder,
                                    rhs_casted,
            ...
            BinLogical(expr) => {
                match (
                    ...
                    (CodeValue::Value(val_lhs), CodeValue::Value(val_rhs)) => {
                        ...
                        unsafe {
                            match expr.op {
                                LogicalAndOp => Ok(CodeValue::Value(Val(
                                    val_lhs.get_type(),
                                    LLVMBuildAnd(
                                        generator.builder,
            ...
            UnNot(expr) => match (&*expr.rhs).gencode(generator)? {
                CodeValue::Value(val) => unsafe {
                    Ok(CodeValue::Value(Val(
                        val.get_type(),
                        LLVMBuildNot(
                            generator.builder,
                ...
                match (
                    ...
                    (CodeValue::Value(val_lhs), CodeValue::Value(val_rhs)) => {
                        ...
                        unsafe {
                            ...
                            match expr.op {
                                GreaterThanOp => Ok(CodeValue::Value(Val(
                                    decaf_bool_type!(),
                                    LLVMBuildICmp(
                                        generator.builder,
                                        LLVMIntSGT,
                                ...
                                GreaterOrEqOp => Ok(CodeValue::Value(Val(
                                    decaf_bool_type!(),
                                    LLVMBuildICmp(
                                        generator.builder,
                                        LLVMIntSGE,
                                ...
                                LessThanOp => Ok(CodeValue::Value(Val(
                                    decaf_bool_type!(),
                                    LLVMBuildICmp(
                                        generator.builder,
                                        LLVMIntSLT,
                                ...
                                LessOrEqOp => Ok(CodeValue::Value(Val(
                                    decaf_bool_type!(),
                                    LLVMBuildICmp(
                                        generator.builder,
                                        LLVMIntSLE,
                                ...
                                EqualsOp => Ok(CodeValue::Value(Val(
                                    decaf_bool_type!(),
                                    LLVMBuildICmp(
                                        generator.builder,
                                        LLVMIntEQ,
                                ...
                                NotEqualsOp => Ok(CodeValue::Value(Val(
                                    decaf_bool_type!(),
                                    LLVMBuildICmp(
                                        generator.builder,
                                        LLVMIntNE,
                    ...
                    match generator.get_top_most_function_val() {
                        Some(func_val) => {
                            ...
                            if is_class {
                                ...
                                ctor_func_val = Some(
                                    generator
                                        .get_function_by_name(&cls.get_default_ctor().llvm_name())
                                        .unwrap(),
                                );
                            }
                            ...
                            for dim in expr.dims.iter() {
                                match dim.gencode(generator)? {
                                    CodeValue::Value(dim_val) => dim_vals.push(dim_val),
                                    _ => {
                            ...
                            unsafe {
                                for (ix, dim_val) in dim_vals.iter().enumerate() {
                                    if ix == 0 {
                                        ...
                                        let ty =
                                            generator.get_llvm_type_from_decaf_type(&decaf::Type {
                                                base: expr.ty.clone(),
                                                array_lvl: lvl,
                                            });
                                        let ty_inner =
                                        ...
                                        LLVMBuildStore(generator.builder, dim_llvm_val, sz_addr);
                                        ...
                                        LLVMBuildStore(generator.builder, array_var, element_addr);
                                ...
                                let real_next_bb = if is_class {
                                    ...
                                    LLVMBuildStore(generator.builder, obj_addr, obj_addr_addr);
                                ...
                                // Reposition
                                LLVMPositionBuilderAtEnd(generator.builder, real_next_bb);
                            }
            ...
            Literal(expr) => match expr {
                IntLiteral(i) => Ok(CodeValue::Value(Val(
                    decaf::Type {
                        base: IntTy,
                        array_lvl: 0,
                    },
                ...
                BoolLiteral(b) => Ok(CodeValue::Value(Val(
                    decaf::Type {
                        base: BoolTy,
                        array_lvl: 0,
                    },
                ...
                CharLiteral(c) => Ok(CodeValue::Value(Val(
                    decaf::Type {
                        base: CharTy,
                        array_lvl: 0,
                    },
                ...
                StrLiteral(_) => panic!("StrLiteral should not not be accessed"),
            },
            This(_) => {
            ...
            CreateObj(expr) => {
                ...
                match expr.cls.name == "String" {
                    true => {
                        if expr.args.len() == 1 {
                            match &expr.args[0] {
                                Literal(StrLiteral(s)) => Ok(CodeValue::Value(Val(
                                    decaf::Type {
                    ...
                    false => {
                        match generator
                            .get_function_by_name(&expr.ctor.as_ref().unwrap().llvm_name())
                        {
                            Some(ctor_func_val) => {
                                // Allocate first
                                let this;
                                unsafe {
                                    ...
                                    let virtual_table_val = generator
                                        .get_global_by_name(&vtable_name!(expr.cls.name))
                                        .unwrap();

                                    let virtual_table_addr = LLVMBuildGEP(
                                        generator.builder,
                                        this,
                                        generator.indices(vec![0, 0]).as_mut_slice().as_mut_ptr()
                                            as *mut _,
                                        2,
                                        generator.next_name(&format!(
                                            "get_vtable_addr_{}",
                                            &expr.cls.name
                                        )),
                                    );
                                ...
                                for (ix, arg) in expr.args.iter().enumerate() {
                                    match arg.gencode(generator)? {
                                        CodeValue::Value(arg_val) => {
                                            ...
                                            let casted_val = generator.cast_value(
                                                &arg_val.get_type(), // source type
                                                &expr.ctor.as_ref().unwrap().args.borrow()[ix]
                                                    .1
                                                    .as_ref(), // destination type
                                                arg_llvm_val,        // source value
                                            );
                                ...
                                unsafe {
                                    Ok(CodeValue::Value(Val(
                                        ...
                                        LLVMBuildCall(
                                            ...
                                            arg_vals.len() as u32,
                                            generator
                                                .next_name(&format!("ctor_{}", &expr.cls.name)),
                                        ),
                                    )))
                                }
                            }
                            ...
                            CodeValue::Value(val) => {
                                let cls = {
                                    ...
                                    if let ClassTy(c) = &val_type.base {
                                        if val_type.array_lvl == 0 {
                                            ...
                                            return Err(EArrayNotSupportMethodCall(format!(
                                                "{}",
                                                val_type
                                            )));
                                        }
                                    } else {
                                        return Err(ENonClassNotSupportMethodCall(format!(
                                            "{}",
                                            val_type
                                        )));
                                    }
                                ...
                                unsafe {
                                    ...
                                    let func_idx = match cls.vtable.borrow().indexof(&expr.method) {
                                        Some(ix) => ix,
                                        None => {
                                            return Err(EMethodNotFoundInVTable(format!(
                                                "{}",
                                                expr.method
                                            )))
                                        }
                                    ...
                                    let virtual_table_ptr = LLVMBuildLoad(
                                        ...
                                        generator
                                            .next_name(&format!("load_vtable_ptr_{}", &cls.name)),
                                    );
                                    ...
                                    let func_addr = LLVMBuildGEP(
                                        ...
                                        virtual_table_ptr,
                                        generator
                                            .indices(vec![0, func_idx as i64])
                                            .as_mut_slice()
                                            .as_mut_ptr()
                                            as *mut _,
                                        2,
                                        generator
                                            .next_name(&format!("get_func_addr_{}", &cls.name)),
                                    );
                                    ...
                                    let func_val = LLVMBuildLoad(
                                        ...
                                        func_addr,
                                        generator
                                            .next_name(&format!("load_func_val_{}", &cls.name)),
                                    );
                                    ...
                                    let func_val = LLVMBuildBitCast(
                                        ...
                                        func_ty,
                                        generator
                                            .next_name(&format!("bicast_func_val_{}", &cls.name)),
                                    );
                    ...
                    CodeValue::Value(var_val) => {
                        ...
                        let (field_idx, field_ty, cls) = {
                            if let ClassTy(cls) = obj_ty.base {
                                if obj_ty.array_lvl == 0 {
                                    ...
                                    let field_ty =
                                        generator.ptr_if_class_or_array(&*expr.fld.ty.borrow());
                                    ...
                                    (
                                        1,
                                        generator.int_type(),
                                        generator.class_lookup("Array").unwrap(),
                                    ) // Array does not have vtable ptr
                                }
                            } else {
                                if obj_ty.array_lvl > 0 {
                                    (
                        ...
                        unsafe {
                            let field_addr = LLVMBuildGEP(
                                ...
                                obj_val,
                                generator
                                    .indices(vec![0, field_idx as i64])
                                    .as_mut_slice()
                                    .as_mut_ptr() as *mut _,
                                2,
                                generator.next_name(&format!(
                                    "gep_field_addr_{}_{}",
                                    cls.name, expr.fld.name
                                )),
                            );
            ...
            decaf::Expr::Variable(expr) => Ok(CodeValue::Value(Addr(expr.var.clone()))),
            ClassId(expr) => Ok(Type(
                decaf::Type {
                    base: ClassTy(expr.cls.clone()),
                    array_lvl: 0,
                },
